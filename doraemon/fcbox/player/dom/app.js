...
const FcDomPlayer = {
  ...
  methods: {
    ...
    touchmove (e) {
      const scrollParam = this.scrollParam
      if (!scrollParam.moveLock) {
        return
      }
      const event = (!e.pageX && !e.x) ? e.targetTouches[0] : e
      scrollParam.mX = event.pageX || event.x
      scrollParam.mY = event.pageY || event.y
      if (Math.abs(scrollParam.mX - scrollParam.sX) > 10 || (Math.abs(scrollParam.mX - scrollParam.sX) > Math.abs(scrollParam.mY - scrollParam.sY)) || scrollParam.move) {
        e.preventDefault()
